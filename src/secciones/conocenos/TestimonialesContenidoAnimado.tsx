...
function PerfilTestimonialSeccion({
  perfil,
  index,
  i18nPrefix,
}: {
  perfil: PerfilTestimonial;
  index: number;
  i18nPrefix: string;
}) {
  const { t } = useTranslation();
  const prefix = `${i18nPrefix}.profiles.${perfil.id}` as const;
          ...
          <div className="grid items-center gap-10 lg:grid-cols-12 lg:gap-12">
            ...
            >
              ...
              <div
                ...
                aria-hidden
              />
              <p className="mt-6 text-base font-medium italic text-iex-navy/90 md:text-lg">
                «{t(`${prefix}.opinionLead`)}»
              </p>
              <p className="mt-4 text-base leading-relaxed text-muted-foreground md:text-lg">
                {t(`${prefix}.opinionBody`)}
              </p>
...
type TestimonialesContenidoAnimadoProps = {
  i18nPrefix: string;
  perfiles: readonly PerfilTestimonial[];
};

export function TestimonialesContenidoAnimado({
  i18nPrefix,
  perfiles,
}: TestimonialesContenidoAnimadoProps) {
  const { t } = useTranslation();
          ...
          <Reveal y={20}>
            <div className="mx-auto max-w-3xl text-center lg:mx-0 lg:max-w-2xl lg:text-left">
              ...
              <p className="mt-6 text-base leading-relaxed text-muted-foreground md:text-lg">
                {t(`${i18nPrefix}.intro.lead`)}
              </p>
              <p className="mt-4 text-base leading-relaxed text-muted-foreground md:text-lg">
                {t(`${i18nPrefix}.intro.body`)}
              </p>
            </div>
          </Reveal>
        </div>
